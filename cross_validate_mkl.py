"""
Cross-validation for the MKL kernel methods pipeline.

Searches over:
  - kernel type per group (rbf / chi2)
  - gamma multipliers per group
  - lambda (KRR regularisation)
  - MKL weights
  - Nystrom m
...
Results are printed and saved to cv_results.csv.
"""
...
# Augmentation during CV (set False to speed up)
DO_AUGMENT   = True
AUG_FLIPS    = True
AUG_SHIFTS   = True
AUG_SHIFT_PX = 2

# Nystrom for CV (lower = faster, less accurate)
NYSTROM_M = 1000
...
# Gamma multipliers applied to auto-gamma = 1/(D*var)
# e.g. 0.5 means gamma = 0.5 * auto_gamma
GAMMA_MULTS = [0.1, 0.5, 1.0, 2.0, 5.0]

# Kernel type per group: 'rbf' or 'chi2'
# chi2 is often better for histogram features (LBP, color)
KERNEL_CONFIGS = [
...
# MKL weights: (w_hog, w_lbp, w_color), will be normalised to sum=1
WEIGHT_CONFIGS = [
...
# ─────────────────────────────────────────────
#  Load data + extract features once
# ─────────────────────────────────────────────
...
def make_kernel_fns(kernel_config, gamma_mult, groups_train_n):
    """Build kernel_fns dict with per-group gamma."""
    fns = {}
    ...
    for fold_i, (train_idx, val_idx) in enumerate(folds):
        ...
        if DO_AUGMENT:
            ...
            imgs_aug, y_aug = augment_dataset(
                ...
                shift=AUG_SHIFT_PX)
            # Extract features for augmented images only (the extra ones)
        ...
        # Build kernels with per-group auto-gamma * gamma_mult
