...
New: MKLRidgeClassifier — Multiple Kernel Learning via KRR.
     Combines one kernel per feature group with learned or fixed weights.
"""
...
# ─────────────────────────────────────────────
#  MKL — Multiple Kernel Learning via KRR
# ─────────────────────────────────────────────
...
class MKLRidgeClassifier:
    ...
    Parameters
    ----------
    lam        : KRR regularisation
    kernel_fns : dict {group_name: callable(X1,X2)->K}
    weights    : dict {group_name: float} or None (uniform)
    nystrom_m  : int or None — Nystrom approximation per group
    """
    ...
    def _build_kernel(self, groups_A, groups_B, use_anchors=False):
        ...
        groups_A, groups_B: dicts {name: array}
        """
    ...
    def fit(self, groups, y):
        ...
        if self.nystrom_m is None or self.nystrom_m >= n:
            ...
            K_nm = self._build_kernel(groups, anchors, use_anchors=True)   # (n, m)

            # Build K_mm using anchors
